    ...
    for (let y = 0; y < rows; y++) {
        ...
        for (let x = 0; x < cols; x++) {
            map[y][x] = {
                ...
                resourceYield: { food: 1 } // Grass yields 1 food
            };
        }
    }
...
function addWaterFeatures(map: Tile[][], rows: number, cols: number): void {
    ...
    for (let y = 0; y < rows; y++) {
        ...
        map[y][riverX].resourceYield = {}; // Water yields no resources
        ...
        if (riverX + offset >= 0 && riverX + offset < cols) {
            ...
            map[y][riverX + offset].resourceYield = {}; // Water yields no resources
        }
    }
        ...
        for (let x = lakeX - lakeSize; x <= lakeX + lakeSize; x++) {
            if (y >= 0 && y < rows && x >= 0 && x < cols) {
                ...
                if (distance <= lakeSize) {
                    ...
                    map[y][x].resourceYield = {}; // Water yields no resources
                }
            }
        }
    }
                ...
                if (y >= 0 && y < rows && x >= 0 && x < cols) {
                    ...
                    if (map[y][x].terrainType !== TerrainType.WATER) {
                        ...
                        if (distance <= forestSize && Math.random() > 0.3) {
                            ...
                            map[y][x].resourceYield = { wood: 1 }; // Forest yields 1 wood
                        }
                    }
                }
            }
            ...
            if (x >= 0 && y >= 0 && y < rows && x < cols) {
                ...
                if (map[y][x].terrainType !== TerrainType.WATER) {
                    ...
                    if (noise > 0.7 || (x === mountainRangeStart + offset && noise > 0.3)) {
                        ...
                        map[y][x].resourceYield = {}; // Mountains yield no resources
                    }
                }
            }
        }
